...
public class Shoe {
    ...
    {
        break; // Must have a -constructor- in which the number of decks of cards is specified as a parameter (number of decks from 6 or 8) any other value = CardExeption.
    ...
    public shuffle()
    {
        return shuffle; // re orders the cards of the shoe randomly use "COLLECTIONS utility class" from "JAVA.UTIL" package. 
    }

    public Card deal()
    {
        return deal; // removes the first stored card and returns it to the caller. if deals from empty shoe = CardExeption.  
    }
